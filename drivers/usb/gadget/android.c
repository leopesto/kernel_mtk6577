...
//This funciton will replace "^" to " " in .ProjectConfig.mk  Manufacture String. Product String
void HQ_usb_name_replace(char *pReplace){
	unsigned int len = 0;
	unsigned int i = 0;
	len = strlen(pReplace);
	if (len > 100) len = 100; // 
...
#ifdef HQ_USB_MANUFACTURER_NAME
#define MANUFACTURER_STRING HQ_USB_MANUFACTURER_NAME
#else
...
#ifdef HQ_USB_PRODUCT_NAME
#define PRODUCT_STRING      HQ_USB_PRODUCT_NAME
#else
#define PRODUCT_STRING      "MT65xx Android Phone"
#endif


#ifdef HQ_USB_MS_VENDOR
#define USB_MS_VENDOR       HQ_USB_MS_VENDOR
#else
#define USB_MS_VENDOR 		"Linux"
#endif


#ifdef HQ_USB_MS_PRODUCT
#define USB_MS_PRODUCT      HQ_USB_MS_PRODUCT
#else
#define USB_MS_PRODUCT      "File-Stor Gadget"
#endif


static char usb_ms_vendor[9]; //only 8 bytes
static char usb_ms_product[17]; //only 16 bytes
...
#define LUN_NUMBER (NLUN_CD_ROM + NLUN_STORAGE)
	...
	//kaka_12_0417 add
	strncpy(usb_ms_vendor,USB_MS_VENDOR,8);
	HQ_usb_name_replace(usb_ms_vendor);
								
	strncpy(usb_ms_product,USB_MS_PRODUCT,16);
	HQ_usb_name_replace(usb_ms_product);

	config->fsg.vendor_name = usb_ms_vendor;
	config->fsg.product_name = usb_ms_product;
	//kaak_12_0417 end
...
static int android_bind(struct usb_composite_dev *cdev)
{
	...
	//kaka_12_0417 add
    HQ_usb_name_replace(manufacturer_string);
    HQ_usb_name_replace(product_string);
    //kaka_12_0417 end
