            ...
            pm_message_t state)
{
    ...
    if (initcall_debug) {
        rettime = ktime_get();
        delta = ktime_sub(rettime, calltime);
        pr_debug("initcall %s_i+ returned %d after %Ld usecs\n",
            dev_name(dev), error,
...
static int device_resume(struct device *dev, pm_message_t state, bool async)
{
    ...
    if (dev->pwr_domain) {
        ...
        printk(KERN_DEBUG "[%d] power domain device_resume pass\n",device_resume_index++);
        goto End;
    }
...
static int __device_suspend(struct device *dev, pm_message_t state, bool async)
{
    ...
    if (dev->pwr_domain) {
        pm_dev_dbg(dev, state, "power domain ");
        printk(KERN_EMERG "[%d] power domain device_suspend\n", device_suspend_index);
        //if (dev->driver)
        //	printk("dev->driver->suspend: %p\n", dev->driver->suspend);
        if (dev->driver)
        	if(dev->driver->name)
            printk("dev->driver->name=%s\n", dev->driver->name);
        error = pm_op(dev, &dev->pwr_domain->ops, state);
        printk(KERN_EMERG "[%d] power domain device_suspend pass\n", device_suspend_index++);
        goto End;
    }

    if (dev->type && dev->type->pm) {
        pm_dev_dbg(dev, state, "type ");
        printk(KERN_EMERG "[%d] type device_suspend\n", device_suspend_index);
        //if (dev->driver)
        //	printk("dev->driver->suspend: %p\n", dev->driver->suspend);
        if (dev->driver)
        	if(dev->driver->name)
            printk("dev->driver->name=%s\n", dev->driver->name);
        error = pm_op(dev, dev->type->pm, state);
        printk(KERN_EMERG "[%d] type device_suspend pass\n", device_suspend_index++);
        goto End;
    }
    ...
    if (dev->class) {
        if (dev->class->pm) {
            pm_dev_dbg(dev, state, "class ");
            printk(KERN_EMERG "[%d] class device_suspend\n", device_suspend_index);
            //if (dev->driver)
            //	printk("dev->driver->suspend: %p\n", dev->driver->suspend);
            if (dev->driver)
            	if(dev->driver->name)
            		printk("dev->driver->name=%s\n", dev->driver->name);
        		error = pm_op(dev, dev->class->pm, state);
            printk(KERN_EMERG "[%d] class device_suspend pass\n", device_suspend_index++);
            goto End;
        } else if (dev->class->suspend) {
            pm_dev_dbg(dev, state, "legacy class ");
            printk(KERN_EMERG "[%d] legacy class device_suspend\n", device_suspend_index);
            //if (dev->driver)
            //	printk("dev->driver->suspend: %p\n", dev->driver->suspend);
            if (dev->driver)
            	if(dev->driver->name)
            ...
            printk(KERN_EMERG "[%d] legacy class device_suspend pass\n", device_suspend_index++);
    ...
    if (dev->bus) {
        if (dev->bus->pm) {
            pm_dev_dbg(dev, state, "");
            printk(KERN_EMERG "[%d] device_suspend\n", device_suspend_index);
            //if (dev->driver)
            //	printk("dev->driver->suspend: %p\n", dev->driver->suspend);
            if (dev->driver)
            	if(dev->driver->name)
            		printk("dev->driver->name=%s\n", dev->driver->name);
        		error = pm_op(dev, dev->bus->pm, state);
            printk(KERN_EMERG "[%d] device_suspend pass\n", device_suspend_index++);
        } else if (dev->bus->suspend) {
            pm_dev_dbg(dev, state, "legacy ");
            printk(KERN_EMERG "[%d] legacy device_suspend\n", device_suspend_index);
            //if (dev->driver)
            //	printk("dev->driver->suspend: %p\n", dev->driver->suspend);
            if (dev->driver)
            	if(dev->driver->name)
            ...
            printk(KERN_EMERG "[%d] legacy device_suspend pass\n", device_suspend_index++);
