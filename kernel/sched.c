...
//
//Update Patch
//http://git.kernel.org/?p=linux/kernel/git/torvalds/linux.git;a=commitdiff;h=a3e5d1091c1298ded486aba87c22fe90cb55ea6c
//
...
//
//Update Patch
//http://git.kernel.org/?p=linux/kernel/git/torvalds/linux.git;a=commitdiff;h=2baab4e90495ebc9826c93f79d74d6e60a828d24
//
...
out:
	if (state != cpuset) {
		/*
		* Don't tell them about moving exiting tasks o
		* kernel threads (both mm NULL), since they never
		* leave kernel.
		*/
		if (p->mm && printk_ratelimit()) {
...
try_to_wake_up(struct task_struct *p, unsigned int state, int wake_flags)
{
	...
	if (task_cpu(p) != cpu) {
		...
		mt_lbprof_printf(MT_LBPROF_TASK, "wakeup task:  %4d %4d %16s %2d %2d\n", p->pid, p->tgid, p->comm, task_cpu(p), cpu);
		set_task_cpu(p, cpu);
	}
#endif /* CONFIG_SMP */
...
//
//Update Patch
//http://git.kernel.org/?p=linux/kernel/git/torvalds/linux.git;a=commitdiff;h=c350a04efd1c89cd256b2abc8f07a21d0d53ff24
//
...
void wake_up_new_task(struct task_struct *p)
{
	...
	mt_lbprof_printf(MT_LBPROF_TASK, "fork task:    %4d %4d %16s %2d %2d\n", p->pid, p->tgid, p->comm, task_cpu(p), dest_cpu);
...
//
//Update Patch
//http://git.kernel.org/?p=linux/kernel/git/torvalds/linux.git;a=commitdiff;h=c308b56b5398779cd3da0f62ab26b0453494c3d4
//
...
void sched_exec(void)
{
	...
	mt_lbprof_printf(MT_LBPROF_TASK, "exec task:    %4d %4d %16s %2d %2d\n", p->pid, p->tgid, p->comm, task_cpu(p), dest_cpu);
 ...
 */
//Update Patch
//http://git.kernel.org/?p=linux/kernel/git/torvalds/linux.git;a=commitdiff;h=6135fc1eb4b1c9ae5f535507ed59591bab51e630
//http://git.kernel.org/?p=linux/kernel/git/torvalds/linux.git;a=commitdiff;h=1c2927f18576d65631d8e0ddd19e1d023183222e
//
...
//
//Update Patch
//http://git.kernel.org/?p=linux/kernel/git/torvalds/linux.git;a=commitdiff;h=3c7d51843b03a6839e9ec7cda724e54d2319a63a
//
...
//
//Update Patch
//http://git.kernel.org/?p=linux/kernel/git/torvalds/linux.git;a=commitdiff;h=1c4dd99bed5f6f70932bf8dacdd54d04a2619778
//
...
void __sched io_schedule(void)
{
	...
	current->in_iowait = 1;
  if(unlikely(ioschedule_dump()))
	{
    AddStorageTrace(STORAGE_LOGGER_MSG_IOSCHED1, io_schedule,
                                                          rq->nr_iowait,
                                                          0, 
                                                          0,
	...
	if(unlikely(ioschedule_dump()))
	{
    AddStorageTrace(STORAGE_LOGGER_MSG_IOSCHED2, io_schedule,
                                                          rq->nr_iowait,
                                                          0, 
                                                          0,
                                                          current->se.statistics.iowait_count, 
                                                          current->se.statistics.iowait_sum);
  }
...
static int __migrate_task(struct task_struct *p, int src_cpu, int dest_cpu)
{
	 ...
	 */
	if (p->on_rq) {
		mt_lbprof_printf(MT_LBPROF_TASK, "migrate task: %4d %4d %16s %2d %2d\n", p->pid, p->tgid, p->comm, task_cpu(p), dest_cpu);
...
//
//Update Patch
//http://git.kernel.org/?p=linux/kernel/git/torvalds/linux.git;a=commitdiff;h=4d78a2239e393f09e0964a2f8da394cc91d75155
//
static int
...
//
//Update Patch
//http://git.kernel.org/?p=linux/kernel/git/torvalds/linux.git;a=commitdiff;h=fb2cf2c660971bea0ad86a9a5c19ad39eab61344
//
static void __sdt_free(const struct cpumask *cpu_map)
{
	struct sched_domain_topology_level *tl;
...
/* EXPORT bellow APIs and provide CPU time/idle information */
