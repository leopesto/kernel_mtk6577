...
static int printk_disable_uart = 0;
...
#define CONSOLE_OUTPUT_MAX_SIZE (25 * 1024)
#define CONSOLE_OUTPUT_MAX_SIZE_STRING "log more than 25Kb, truncate remained\n"
static void emit_log_char(char c);
void emit_log_string(char *str);
		...
		while (cur_index != end) {
			...
			if (c == '\n') {
				...
				if (msg_level < console_loglevel || ignore_loglevel) {
					cut_count += (cur_index - start_print);
					if (cut_count >= CONSOLE_OUTPUT_MAX_SIZE) {
						spin_lock(&cutlock);
						cut_log_flag = 1;
						spin_unlock(&cutlock);
						return;
...
module_param_named(prefix, printk_prefix, int, S_IRUGO | S_IWUSR);
	...
	for (; *p; p++) {
		if (new_text_line) {
			...
			case 2: {
				...
				tlen = snprintf(tbuf, sizeof(tbuf), "(%x, %s, %d)",
...
void resume_console(void)
{
	...
	if (cut_log_flag) {
		cut_log_flag = 0;
		spin_unlock(&cutlock);
		printk(KERN_WARNING CONSOLE_OUTPUT_MAX_SIZE_STRING);
#if 0
		aee_kernel_warning( "too many suspend/resume log","too many suspend/resume log will lead longer resume time.\nconsole level: %d\n",console_loglevel);
#endif
	}
	else {
		spin_unlock(&cutlock);
	}
    printk("[Resume Console] takes %llu ns\n", t2 -t1);
}
